...
exports.handlePrefixCommand = handlePrefixCommand;
...
async function handlePrefixCommand(message) {
    if (!message.guild || !message.member || !message.client.user || message.author.bot)
        return;
    const settings = await CacheManager_1.cache.getGuildSettings(message.guild.id);
    const prefix = settings?.prefix || '$';
    // AFK check & mentions handling
    await afkService_1.afkService.handleMessage(message, prefix);
    ...
    if (command === 'config' && args[0] === 'prefix') {
        ...
        CacheManager_1.cache.invalidateGuildSettings(message.guild.id);
    ...
    if (command === 'rules') {
        ...
        if (sub === 'add') {
            ...
            const res = await rulesService_1.rulesService.addRule(message.guild, message.member, content);
            return message.reply(res.success ? res.message : res.error);
        }
        else if (sub === 'remove') {
            const index = parseInt(args[1]);
            const res = await rulesService_1.rulesService.removeRule(message.guild, message.member, index);
            return message.reply(res.success ? res.message : res.error);
        }
        else {
            const res = await rulesService_1.rulesService.listRules(message.guild);
    ...
    if (command === 'afk') {
        const sub = args[0]?.toLowerCase();
        if (sub === 'remove') {
            const res = await afkService_1.afkService.removeAfk(message.author.id);
            return message.reply(res.success ? res.message : res.error);
        }
        else if (sub === 'status') {
            const res = await afkService_1.afkService.getStatus(message.author.id);
            return message.reply(res.success ? res.message : res.error);
        }
        else {
            const reason = args.join(' ') || 'AFK';
            const res = await afkService_1.afkService.setAfk(message.author.id, reason);
    ...
    if (command === 'permissions') {
        ...
        if (sub === 'add' || sub === 'remove') {
            ...
            const res = sub === 'add'
                ? await permissionsService_1.permissionsService.addPermission(message.guild, message.member, roleId, cmdName)
                : await permissionsService_1.permissionsService.removePermission(message.guild, message.member, roleId, cmdName);
            return message.reply(res.success ? res.message : res.error);
        }
        else if (sub === 'list' || !sub) {
            const res = await permissionsService_1.permissionsService.listPermissions(message.guild);
    ...
    if (command === 'muterole') {
        ...
        if (sub === 'create') {
            ...
            const res = await muteroleService_1.muteroleService.createMuteRole(message.guild, message.member, name);
            return message.reply(res.success ? res.message : res.error);
        }
        else if (sub === 'remove') {
            const res = await muteroleService_1.muteroleService.removeMuteRole(message.guild, message.member);
            return message.reply(res.success ? res.message : res.error);
        }
        else if (args[0]) {
            const roleId = args[0].match(/^<@&(\d+)>$/)?.[1] || args[0];
            const res = await muteroleService_1.muteroleService.setMuteRole(message.guild, message.member, roleId);
            return message.reply(res.success ? res.message : res.error);
        }
        else {
            const res = await muteroleService_1.muteroleService.getMuteRole(message.guild, prefix);
            return message.reply(res.success ? res.message : res.error);
        }
    }
    if (command === 'why' || command === 'history') {
        ...
        if (command === 'why') {
            const res = await intelligence_1.intelligence.getWhyContext(message.guild.id, targetId);
            return res.success ? message.reply({ embeds: [res.embed] }) : message.reply(res.error);
        }
        else {
            const res = await intelligence_1.intelligence.getHistory(message.guild.id, targetId);
            return res.success ? message.reply({ embeds: [res.embed] }) : message.reply(res.error);
        }
    }
    if (command === 'recommend') {
        const res = await intelligence_1.intelligence.getRecommendation(message.guild.id);
        return message.reply({ embeds: [res.embed] });
    }
    if (command === 'dmad') {
        const { dmadCommand } = await Promise.resolve().then(() => __importStar(require('../commands/general/dmad')));
    ...
    if (validModCommands.includes(command)) {
        if (command === 'purge') {
            ...
            const res = await moderationService_1.moderationService.purge(message.client, message.guild, message.member, message.channel, amount);
            ...
            const res = await moderationService_1.moderationService.getCases(message.guild, filterUserId);
            if (res.success) {
            ...
            if (command === 'unban') {
                ...
                const res = await moderationService_1.moderationService.unban(message.client, message.guild, message.member, targetId, reason);
                if (res.success) {
                    await message.reply({ embeds: [res.embed] });
                    commandExecuted = true;
                }
                ...
                if (command === 'warn')
                    action = 'WARN';
                if (command === 'mute' || command === 'tempmute')
                    action = 'TIMEOUT';
                if (command === 'kick')
                    action = 'KICK';
                ...
                if (action) {
                    if (command === 'tempmute' || command === 'mute' || command === 'tempban' || command === 'ban') {
                        if (args.length > 0) {
                            ...
                            if ((0, duration_1.parseDuration)(possibleDuration)) {
                    ...
                    const reason = args.join(' ') || 'No reason provided';
                    const result = await moderationService_1.moderationService.executeAction(message.client, message.guild, message.member, targetUser, action, reason, durationStr);
                    if (!result.success) {
    ...
    if (commandExecuted) {
        const { dmPromotionService } = await Promise.resolve().then(() => __importStar(require('../services/dmPromotionService')));
