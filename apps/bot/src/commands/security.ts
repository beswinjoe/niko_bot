...
export const securityCommand: Command = {
  data: new SlashCommandBuilder()
    ...
    .addSubcommand(subcommand =>
      subcommand.setName('status').setDescription('Show current security state')
    )
    .addSubcommand(subcommand =>
      subcommand.setName('score').setDescription('Show Niko Security Score')
    )
    .addSubcommand(subcommand =>
      subcommand.setName('incidents').setDescription('Show recent security incidents')
    )
    .addSubcommand(subcommand =>
      subcommand.setName('lockdown').setDescription('Enable configured server lockdown protections')
    )
    .addSubcommand(subcommand =>
      subcommand.setName('unlock').setDescription('Disable lockdown')
    ),
      ...
      if (subcommand === 'status') {
        const embed = new EmbedBuilder()
          ...
          .addFields(
            ...
            { name: 'Verification', value: '❌ Disabled', inline: true }
          );
        ...
        const embed = new EmbedBuilder().setColor('#F59E0B').setTitle('Recent Security Incidents');
        ...
        const lockdownPerms = await (prisma as any).roleCommandPermission.findMany({
        ...
        if (!hasConfiguredRole) {
          hasConfiguredRole = lockdownPerms.length === 0 || lockdownPerms.some((p: any) => member.roles.cache.has(p.roleId));
        }
